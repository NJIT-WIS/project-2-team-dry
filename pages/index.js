...
function BlogPreview({ title, description, date, image, slug }) {
  return (
    <div className="max-w-md py-4 px-8 bg-white shadow-lg rounded-lg my-20">
      <div className="flex justify-center md:justify-end -mt-16">
        <img className="w-20 h-20 object-cover rounded-full border-2 border-indigo-500" src={image} alt="blogpost" />
      </div>
      <div>
      ...
      <div className="flex justify-end mt-4">
        <Link href={`/blog/${slug}`}>
          <a className="text-xl font-medium text-indigo-500">Read More</a>
        </Link>
      </div>
    </div>
  );
